...
class AggregatorStepMetric:
    def __init__(self,
                 data_source: pd.DataFrame = None,
                 operation_column: Iterable[str] = None,
                 load_plan: Iterable = None,
    ...
    def metric(self, metric_group: Union[str,list[str]], func: MetricAggregationFunction, column: Optional[str] = None) -> pd.Series:
        ...
        for load_level, shift_time, duration_time in self.shift_time_load():
            ...
            #print(f"[metric] self.data_source time_column: {self.data_source[self.time_column]}")
            #print(f"[metric] start_time + timedelta: {self.start_time + timedelta(minutes=shift_time)}")

            # Time window for this load step
            ...
            # print(f"[metric] mask_time_start: {mask_time_start} mask_time_end: {mask_time_end}")
            # print(f"[metric] self.start_time: {self.start_time}")
